...
-   **Mobile-Responsive Admin Pages:** Admin pages (Quizzes, ContentCountries, Agencies, Users, FindyAI, plus the analytics surfaces in Dashboard/Reports/FindyAI Analytics tab — note: there is no standalone `Analytics.tsx`; analytics live inside Dashboard.tsx, Reports.tsx and the `AnalyticsTab` inside FindyAI.tsx, and they inherit the global Dialog + AdminLayout fixes) are usable on 360–768px viewports without horizontal scroll. Global `DialogContent` shrinks to `w-[calc(100%-1rem)]` with `max-h-[calc(100dvh-2rem)] overflow-y-auto` on mobile. `AdminLayout` topbar/main padding shrinks to `p-3` on mobile and the title is truncated. Admin form grids changed from `grid-cols-2` to `grid-cols-1 sm:grid-cols-2`. Card headers use `flex-wrap items-center justify-between gap-3` so action buttons wrap below titles. List rows in `ContentCountries` (countries + content) and the user-row in `Users` use `flex-wrap gap-3` so the trailing action buttons wrap under the label on narrow screens. The Agencies list-view `<Table>` and the two FindyAI tables (Turkish-mapping list, Knowledge-source list) are wrapped in `overflow-x-auto`. The existing mobile drawer on `AdminLayout` (Menu button + overlay) handles sidebar collapse on small viewports.
