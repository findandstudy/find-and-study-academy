...
- **Partner Zone Folder System:** Google Drive-style folder management. `partner_folders` table (id, name, description, coverImageUrl, countryCode, categoryTag, status, order) + `folderId` FK on `contents`. Admin UI (`/admin/partner-zone`): folder grid view → click → folder detail with content table. Agent UI (`/agent/partner-zone`): cover-image card grid → click → content cards with download/open. Public API: `GET /api/partner-folders`, `GET /api/partner-folders/:id/contents`. Admin API: full CRUD on `/api/admin/partner-folders` + `PATCH /api/admin/contents/:id/folder` to assign content to folders.
